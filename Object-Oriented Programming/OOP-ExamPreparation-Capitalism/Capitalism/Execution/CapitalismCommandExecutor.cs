...
namespace Capitalism.Execution
{
    public class CapitalismCommandExecutor : ICommandExecutor
    {
        ...
        public string ExecuteCommand(ICommand command)
        {
            ...
            {
                case "create-company":
                    commandResult = this.ExecuteCreateCompanyCommand(command);
                    break;
                case "create-department":
                    commandResult = this.ExecuteCreateDepartmentCommand(command);
                    break;
                case "create-employee":
                    commandResult = this.ExecuteCreateEmployeeCommand(command);
                    break;
                case "pay-salaries":
                    commandResult = this.ExecutePaySalariesCommand(command);
                    break;
                case "show-employees":
                    commandResult = this.ExecuteShowEmployeesCommand(command);
                    break;
                default:
        ...
        private string ExecuteCreateDepartmentCommand(ICommand command)
        {
            ...
            if (company == null)
            {
                return string.Format($"Company {command.Parameters[0]} does not exist");
            }
        ...
        {
            string companyName = command.Parameters[2];
            var company = this.database.Companies.FirstOrDefault(c => c.Name == companyName);
            if (company == null)
            {
            ...
            var employee = Activator.CreateInstance(employeeType, args: new object[] { command.Parameters[0], command.Parameters[1] }) as IEmployee;
            if (command.Parameters.Count == 4)
            {
            ...
            else // command.Parameters.Count == 5
            {
                ...
                if (department == null)
                {
                    department = company.Departments.SelectMany(d => d.SubDepartment).FirstOrDefault(sd => sd.Name == departmentName);
                }
                if (department == null)
                {
            ...
            decimal salary = this.salaryManager.GetSalary(employee, company);
        ...
        {
            string companyName = command.Parameters[4];
            var company = this.database.Companies.FirstOrDefault(c => c.Name == companyName);
            if (company == null)
            {
                return string.Format($"Company {companyName} does not exist");
            ...
            {
                totalMoneyPaid = this.PaySalariesToDepartment(department, output, 1);
            }
        ...
        private string ExecuteShowEmployeesCommand(ICommand command)
        {
            ...
            var output = new StringBuilder();
            output.AppendLine($"({company.Name})").AppendLine();
